 ...
 * Date: 10/27/14
 */
public class BlacklistScenarioGenerator extends ScenarioGenerator {
    ...
    private String [] getBlackList() {
        ...
        String [] blackListableRoles = {"assignee", "observer"};

        int nSamplesNeeded = new Random().nextInt(blackListableRoles.length + 1);

        String [] blackList = new String[nSamplesNeeded];


        int nPicked = 0, i = 0, nLeft = blackListableRoles.length;
        while (nSamplesNeeded > 0) {
            int rand = new Random().nextInt(nLeft);
            if (rand < nSamplesNeeded) {
                blackList[nPicked++] = blackListableRoles[i];
                nSamplesNeeded--;
            }
            nLeft--;
            i++;
    ...
    public void insert() {
        ...
        for (int s = 1; s <= totalScenarios; s++) {
            for (int t = 1; t <= totalScenarios; t++) {
                ...
                Map<String, Object> scenarioHashMap = new HashMap<>();
                scenarioHashMap.put("index", s);
                long scenarioNode = inserter.createNode(scenarioHashMap, scenarioSpecificLabel);
                ...
                for (Long origin : scenario.tasksIds) {
                    for (int i = 0; i < sLinkingRatio; i++) {
                        ...
                        long relationShipId = inserter.createRelationship(origin, target, linked, null);
                        inserter.setRelationshipProperty(relationShipId, "blacklist", getBlackList());
                ...
                RelationshipType assignedRel = DynamicRelationshipType.withName("assignee");
                ...
                for (long taskId : scenario.tasksIds) {
                    ...
                    inserter.createRelationship(assignee, taskId, assignedRel, null);
